...
6. 連続 write の最中に**本体の電源を切り**、入れ直して部分状態を読み戻す（6-1 〜 6-4）
...
BLE 接続だけを一瞬で切るのは現実的でないため、**本体の電源スイッチを切る**。
切断と電源断が同時に起きるが、R-005 が知りたい「ack が返ったぶんは残るのか」は
むしろこちらの方が直接測れる。

- 6-1 で layer 9 → 5 を 1 keycode ずつ読み、**現在 `0x0000` の位置だけ**を対象にする。
  非ゼロの位置には触らない。実機では 270 件前後（layer あたり 54 件）
- 6-2 は verify の再 read を挟まずに write だけを送る。往復が倍にならないぶん
  「どこまで ack が返ったか」の境界がぼやけない。BLE で 1 件約 30ms、270 件で 8 秒前後
- 値は index から決める（`0x0004 + i % 90`）。読み戻して位置と値の対応が崩れていれば、
  write が別の位置へ入ったことになる
- 6-3 は uptime を読んでから全対象を読み戻し、残った件数・消えた件数・境界が連続しているかを出す
- 6-4 で対象をすべて `0x0000` へ戻す

6-2 と 6-3 の間でページを再読み込みすると対象の一覧が失われる。
