...
設定は keysync リポジトリ（workspace）へ保存し、Web UI や CLI で編集・検証します。
...
1. リポジトリで `just ui` を実行します。Chrome で <http://127.0.0.1:5178/> が開き、リポジトリの設定が読み込まれます。
...
5. リポジトリ直下に `keymap.yaml`、`keysync/definitions/` に definition ファイルができたことを確認します。
