...
- **backupは`ValidatedApplyInput`の必須フィールド**にする。backupが無ければ計画そのものを
  ...
  を上位のフラグではなく型と引数で守る
- **validationした`VilDocument`からdesiredとwrite targetを同一entrypointで導出し、
  evidenceとfull-read backupを`ValidatedApplyInput`へ束ねる。** evidence constructorは
  exportしない。`createValidatedApplyInput`はgateとbackupだけを受け取り、caller supplied
  desired / target / contextを差し込む経路とcoverage不足を型とpreconditionで塞ぐ
...
- validationのうちどれをApply blockingにするかは、severity modelを決めるD-003 の範囲。
  gate済み入力を`ValidatedApplyInput`へ束ねることで、validation結果をApplyの前提条件へ接続する
