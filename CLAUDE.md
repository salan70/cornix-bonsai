...
以下の共通Skillは `syncing-ai-assets` で `.claude/skills/` へ配備されます。
...
| `syncing-ai-assets`       | ai-assets正本からプロジェクトへのSkill同期 |
| `concise-writing`         | 日英の文章基準と推敲手順                   |
...
| `porting-ai-assets-to-codex` | Claude向け資産からCodex向け資産への移植判断 |
